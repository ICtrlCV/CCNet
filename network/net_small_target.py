...
from network.attention_block import CBAM
from network.model_block import BaseConv, CSPLayer, FocusReplaceConv, SPPFBottleneck
...
class Net(nn.Module):
    def __init__(
        ...
        # CBAM
        # 对应dark5输出的1024维度通道
        self.layer4_cbam = CBAM(int(in_channels[3] * width))
        # 对应dark4输出的512维度通道
        self.layer3_cbam = CBAM(int(in_channels[2] * width))
        # 对应dark3输出的256维度通道
        self.layer2_cbam = CBAM(int(in_channels[1] * width))
        # 对应dark3输出的128维度通道
        self.layer1_cbam = CBAM(int(in_channels[0] * width))
    ...
    def forward(self, x):
        ...
        layer1_cbam = self.layer1_cbam(layer1)
        ...
        fpn_out2 = self.reduce_conv2(f_out1)  # 256->128/32
        # 对输出的基础卷积进行上采样
        f_out2 = self.upsample(fpn_out2)  # 128/4
        # 将dark2输出的特征层与上采样结果进行叠加
        f_out2 = torch.cat([f_out2, layer1_cbam], 1)  # 128->256/4
        # 输出结果到YOLO HEAD
        pan_out3 = self.C3_p2(f_out2)  # 256->128/4

        # 对pan_out3输出结果进行下采样
        p_out2 = self.bu_conv3(pan_out3)  # 128->128/8
        # 将pan_out1输出的下采样结果与CSPLayer输出fpn_out2的结果进行叠加
        p_out2 = torch.cat([p_out2, fpn_out2], 1)  # 128->256/8
        # 对CSPLayer结果进行基础卷积
        # 特征提取（普通卷积 + 标准化 + 激活函数）
        # 输出结果到YOLO HEAD
        pan_out2 = self.C3_n2(p_out2)  # 128->128/8
        ...
        neck_outputs = (pan_out3, pan_out2, pan_out1, pan_out0)
        outputs = []
